...
#
# 0.6에서 0.5로 낮췄다 [2026-09-01 실측, T10.54]. 원래 값에 붙어 있던
# "[제안] 실제 답변으로 조정 여지가 있다"는 단서를 실제 답변으로 닫은 것이다.
#
# 실측 방법: 이 PC의 실제 인덱스(문서 51건·청크 7,970개)에 16개 질문을 앱과
# 같은 경로(hybrid_search → select_excerpts → 프롬프트)로 넣어 받은 **모델이
# 실제로 생성한 문장 27개**를 양성 표본으로 삼고, 같은 문장을 **다른 질문의
# 발췌**와 짝지은 652쌍을 음성 표본으로 삼았다("이 근거로는 뒷받침되지
# 않는다"가 정확히 그 뜻이다 — 내가 지어낸 가짜 문장을 쓰면 실제 모델의
# 말투가 반영되지 않는다).
#
#   양성 최저 0.581 (비정형 기권 1건 0.375 제외) · 음성 최고 0.474
#   → 0.5는 두 분포 사이 빈 구간의 한가운데다.
#   0.50 → 정답 오판 1/27 · 음성 통과 0/652
#   0.60 → 정답 오판 2/27 · 음성 통과 0/652  (옛 값)
#
# 🔴 **낮춰도 검출력이 안 떨어진다는 것이 핵심이다.** 음성 652쌍 중 0.5를
# 넘는 것이 하나도 없어서, 0.6은 검출력을 더 주지 못한 채 정답만 더 걸렀다.
# 오판의 실제 모습은 **표를 근거로 한 환언**이었다 — 표는 서술어 없이 쓰이므로
# ("응시료 | 20만원 | 35만원") 자연스러운 문장을 만들면 "~에 따라"·"또는"·
# "입니다" 같은 접착어가 근거에 없어 구조적으로 점수가 깎인다. 금액·사양 표는
# 이 앱에서 가장 흔한 근거 유형이라 그대로 두면 정답에 계속 배지가 붙는다.
SLM_OVERLAP_THRESHOLD = 0.5
