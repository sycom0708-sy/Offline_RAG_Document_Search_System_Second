...
#
# `(?!\d)`(마침표류 뒤에 숫자가 바로 오면 경계로 안 본다)는 T10.53 이후 추가.
# 원래 `\s*`가 공백 0개도 허용해 "3.2" 같은 소수점 숫자가 "3."·"2" 두 문장으로
# 쪼개졌다(재조립 시 마침표 뒤에 없던 공백이 낌 — 표시 문구만의 문제, FTS5
# unicode61 토크나이저가 마침표를 이미 구분자로 봐서 검색 매칭에는 영향 없다).
# 마침표 뒤에 공백이 있으면(정상적인 문장 종결) 이 전방탐색과 무관하게 그대로
# 경계로 인정된다 — 숫자가 **공백 없이 바로 붙어 있을 때만** 예외로 둔다.
_SENTENCE = re.compile(
    r"""
    .+?                                  # 문장 본문 (최소 매칭)
    (?:
        [.!?]+(?!\d)[\"')\]】」』]*\s*    # 마침표류(소수점 제외) + 닫는 인용/괄호
        | \n+                              # 줄바꿈
        | $                                 # 문자열 끝
    )
    """,
