...
| Phase 6 | sLM 후보군 실측 검증 | ⏸️ **진행 중** | 착수 결정 확정 + 런타임/다운로더 코드까지. **실측 시작 전** (§6-B 중단 지점 참고) |
...
## 6-B. 실행 결과 — ⏸️ **진행 중 (중단 지점 기록)**

> **2026-08-07 중단.** 시간 관계로 여기까지 하고 다음 세션에 이어간다. **아직 실측은 시작 전이며 TASK 체크박스는 하나도 못 채웠다** — 아래는 "무엇이 되어 있고 무엇이 안 됐는지"의 정확한 기록이다.

**되어 있는 것 (코드만, 실행 검증 전)**
- `config/settings.py` — `SlmProfile` 데이터클래스 + 후보 4종 정의(`SLM_CANDIDATES`), `models/slm/` 경로. 기존 376 테스트 회귀 없음 확인
- `slm/runtime.py` — llama-server 탐색(`find_llama_server()`, 환경변수 > PATH > `vendor/` 순)과 수명주기 컨텍스트 매니저(`llama_server()`, `/health` 폴링으로 준비 대기 + 로딩 시간 반환)
- `slm/download.py` — GGUF 다운로더. 건당 1~5GB라 **중단 시 이어받기**(HTTP Range) 구현. `--list`로 후보·크기·설치 여부 확인 가능
- `.gitignore` — `vendor/` 추가

**아직 안 된 것 (다음 세션 착수 지점)**
1. `scripts/setup_llamacpp.py` — 18MB 바이너리 다운로드+압축해제 (**여기부터 시작**)
2. `slm/client.py` — llama-server HTTP 클라이언트(urllib)
3. EXAONE-4.0-1.2B 하나만 받아 종단 응답 확인 → **여기까지 되면 하네스가 산다**
4. `slm/prompt.py`(T6.3) → `scripts/build_slm_testset.py`(T6.4) → `scripts/benchmark_slm.py`(T6.5/T6.6)
5. 나머지 3개 모델 다운로드 → 전체 측정 → 비교표·최종 선정(T6.7)

**주의사항 (다음 세션에 잊지 말 것)**
- 10.8GB를 다 받아놓고 하네스가 안 돌면 시간을 크게 버린다 — **가장 작은 0.81GB 모델 하나로 전 구간을 먼저 통과**시킨 뒤 나머지를 받는다
- Qwen3.5는 thinking 모드가 켜져 있으면 CPU에서 매우 느리다. 측정 전 비활성 여부 확인
- 측정 직전 여유 RAM 확인 — 중단 시점 기준 여유가 3GB뿐이라 4.77GB 모델은 다른 앱을 닫아야 한다
