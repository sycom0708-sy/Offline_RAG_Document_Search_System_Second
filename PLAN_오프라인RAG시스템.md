...
**환경 제약**: 이 기준기에는 LibreOffice가 없어 `.doc`·`.xls` 2건이 **0청크**로 처리됐다(오류 없이 조용히 건너뛴다). Phase 9에서 LibreOffice 포터블을 번들하면 해소되지만, **번들 전에는 구버전 포맷이 검색되지 않는다는 사실이 사용자에게 보이지 않는다** — Phase 9에서 안내 문구를 검토할 것.
