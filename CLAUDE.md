...
- **알려진 실패 1건 (Phase 3 영역)**: `test_batching_gives_practically_equivalent_vectors`가 최소 사양 기준기에서 실패한다(배치 vs 단건 자기 유사도 0.921 < 기준 0.98). 문서 벡터는 배치로, 질의 벡터는 단건으로 만들기 때문에 재순위 품질에 실제 영향이 있을 수 있어 **기준값만 낮추면 안 된다**. int8 양자화의 CPU 의존성 또는 onnxruntime 버전 차이가 의심된다.
